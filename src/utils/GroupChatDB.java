...
public class GroupChatDB {
	...
	public Vector<GroupChat> getGroupJoin() {
		...
		Statement stmt = null;
		String sql = "select group_chat.* from group_chat join group_member on group_chat.grchat_id = group_member.grchat_id where group_member.user_id ="
				+ u.getID();
	...
	public void getMember(GroupChat gr) {
		...
		// String sql = "SELECT user_id from group_member where grchat_id='" +
		// gr.getID() + "'";
		try {
			cnt.setAutoCommit(false);
			stmt = cnt.createStatement();
	...
	public boolean adminStatus(GroupChat gr) {
		String check = "SELECT user_admin from group_member where grchat_id='" + gr.getID() + "' and user_id="
				+ u.getID();
		Statement stmt = null;
	...
	public void giveAdmin(GroupChat gr, String username) {
		...
		try {
			...
			if (rs.next()) {
				...
				String upsql = "UPDATE group_member SET user_admin=true where user_id=" + id + " and grchat_id='"
						+ gr.getID() + "'";
	...
	public ChatMessage SaveMessage(String msg, GroupChat gr) {
		...
		try {
			cnt.setAutoCommit(false);
			stmt = cnt.createStatement();
			String sql = "INSERT INTO group_chatmessage VALUES('" + gr.getID() + "','" + dtf.format(now) + "',"
					+ u.getID() + ",'" + msg + "')";
			stmt.executeUpdate(sql);
			cnt.commit();
		} catch (SQLException e) {
	...
	public void DeleteMessage(ChatMessage del) {
		...
		try {
			cnt.setAutoCommit(false);
			stmt = cnt.createStatement();
			String sql = "INSERT INTO erased_group_chatmessage VALUES('" + del.getID() + "','" + del.getMessage_date()
	...
	public void GetMessage(GroupChat gr, DefaultTableModel chat) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery(sql);
			while (rs.next()) {
				ChatMessage chathis = new ChatMessage(rs.getString(1), rs.getString(2),
